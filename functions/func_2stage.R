func_2stage <- function(mnar_data, mar_data, method, m, n, k, bk){

  #### stage 1: impute MNAR for m imputations
  mnar <- subset(mnar_data, select=-c(AGE6, AGE7, AGE8, AGE9, framid))
  mar <- subset(mar_data, select=-c(AGE6, AGE7, AGE8, AGE9, framid))
  ...
  for (i in 1:imp_alldata$m) {
    ...
    imp_mnar_sub_cont <- subset(imp_mnar_sub_cont, select = -c(AGE5, SEX))
    imp_mnar_sub_bin <- imp_mnar_sub[,grepl("CURRSMK", names(imp_mnar_sub))]
    imp_mnar_sub_bin <- cbind(imp_mnar_sub_bin, imp_mnar_sub[,c("SEX", "AGE5")])
    
    setDF(mnar)
    mnar_cont <- mnar[,!grepl("CURRSMK", names(mnar))]
    mnar_cont <- subset(mnar_cont, select = -c(AGE5, SEX))
    mnar_bin <- mnar[,grepl("CURRSMK", names(mnar))]
    mnar_bin <- cbind(mnar_bin, mnar[,c("SEX", "AGE5")])
    ...
    logistic6 <- glm(CURRSMK6~CURRSMK5+CURRSMK7+CURRSMK8+CURRSMK9+AGE5+SEX
    ...
    logistic7 <- glm(CURRSMK7~CURRSMK5+CURRSMK6+CURRSMK8+CURRSMK9+AGE5+SEX
                     +BMI5+BMI6+BMI7+BMI8+BMI9+SBP5+SBP6+SBP7+SBP8+SBP9
                     +DBP5+DBP6+DBP7+DBP8+DBP9+TC5+TC6+TC7+TC8+TC9
                     +FASTING_BG5+FASTING_BG6+FASTING_BG7+FASTING_BG8+FASTING_BG9, family="binomial", data=imp_mnar_sub)
    ...
    logistic8 <- glm(CURRSMK8~CURRSMK5+CURRSMK6+CURRSMK7+CURRSMK9+AGE5+SEX
                     +BMI5+BMI6+BMI7+BMI8+BMI9+SBP5+SBP6+SBP7+SBP8+SBP9
                     +DBP5+DBP6+DBP7+DBP8+DBP9+TC5+TC6+TC7+TC8+TC9
                     +FASTING_BG5+FASTING_BG6+FASTING_BG7+FASTING_BG8+FASTING_BG9, family="binomial", data=imp_mnar_sub)
    ...
    logistic9 <- glm(CURRSMK9~CURRSMK5+CURRSMK6+CURRSMK7+CURRSMK8+AGE5+SEX
                     +BMI5+BMI6+BMI7+BMI8+BMI9+SBP5+SBP6+SBP7+SBP8+SBP9
                     +DBP5+DBP6+DBP7+DBP8+DBP9+TC5+TC6+TC7+TC8+TC9
                     +FASTING_BG5+FASTING_BG6+FASTING_BG7+FASTING_BG8+FASTING_BG9, family="binomial", data=imp_mnar_sub)
    ...
    imp_mnar_sub <- cbind(imp_mnar_sub_bin[,-c(6,7)], imp_mnar_sub_cont)
    setDT(imp_mnar_sub)
    imp_mnar_sub <- cbind(imp_mnar_sub, mnar$SEX, mnar$AGE5) ### same order as in pred
    colnames(imp_mnar_sub) <- c(paste0("CURRSMK", 5:9), paste0("BMI", 5:9), paste0("SBP", 5:9), paste0("DBP", 5:9),
                                paste0("TC", 5:9), paste0("FASTING_BG", 5:9), "SEX", "AGE5")
    ...
    imp_all$framid <- rep(alldata2$framid, 5)
